(function () {
    ...
    angular
        .module('vk')
        .controller('HeyTeaRulesController', HeyTeaRulesController);

    HeyTeaRulesController.$inject = ['$location', '$sessionStorage', 'lodash', '$templateCache', '$timeout', 'MyService','LoginService','$sce'];

    /** @ngInject */
    function HeyTeaRulesController($location, $sessionStorage, lodash, $templateCache, $timeout, MyService,LoginService,$sce) {
        ...
        function init() {
            ...
            MyService.getMemberInfo(cardId, token).then(function (res) {
                ...
                MyService.getMemberLevelDetail(cardId, storeId, token).then(function (data) {
                    ...
                    var dict = {levelName:'规则说明',id:'1',privilegeText:detail};
                    data.data.splice(0, 0, dict);
        ...
        function setBackground(data) {
            lodash.forEach(data, function (v, k) {
                ...
                v.detail = $sce.trustAsHtml( v.privilegeText);
